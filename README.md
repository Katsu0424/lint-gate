...
2. **oxlint プリセット + 自作ルール** — サイズ・複雑度・構造の上限(`complexity: 10` / 認知的複雑度 15 / `max-lines: 400` / 関数 80 行など)、`@ts-ignore` 系の禁止、放置 Promise の検出、focused / skipped テストの禁止、sonarjs 相当 4 ルール(同一関数・全分岐同一・要素上書き・不変 return)、レイヤ境界(`src/domain/` の I/O 禁止)
