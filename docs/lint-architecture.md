...
| 分類                                | ルール                                                                                                                                                                                                                                                        |
| ----------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| サイズ・複雑度                      | `complexity` 10 / `lint-gate/cognitive-complexity` 15 / `max-depth` 4 / `max-lines` 400 / `max-lines-per-function` 80 / `max-nested-callbacks` 4 / `max-params` 4 / `max-statements` 30(`test/` `tests/` `*.test.ts(x)` `*.spec.ts(x)` では行数系 3 つを off) |
...
| 型の誤解・古い API(型情報)          | `typescript/no-unnecessary-condition`(型から結果が確定する死に分岐)/ `no-deprecated` / `return-await`(in-try-catch)/ `require-await` / `prefer-nullish-coalescing`                                                                                            |
| 比較・宣言・ループの基本            | `eqeqeq`(`== null` は許可)/ `prefer-const` / `no-var` / `no-shadow` / `no-loop-func` / `radix` / `no-template-curly-in-string` / `array-callback-return`                                                                                                      |
| Promise チェーン                    | `promise/catch-or-return`(catch で終わらない then チェーン)                                                                                                                                                                                                   |
| 非 null 断定                        | `typescript/no-non-null-assertion`(ソースのみ。テストファイルは off)                                                                                                                                                                                          |
| テストの形骸化                      | `vitest/expect-expect` / `no-focused-tests` / `no-disabled-tests` / `no-identical-title`                                                                                                                                                                      |
